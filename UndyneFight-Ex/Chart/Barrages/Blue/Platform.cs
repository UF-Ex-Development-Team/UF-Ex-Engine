...
public class Platform : Entity, ICustomMotion, ICustomLength
{
	...
	internal class PlatformShinyEffect : Entity
	{
		private readonly Platform following;
		private readonly float length;
		private readonly int platformType;
		private float alpha = 1f, size = 1f;
		public PlatformShinyEffect(Platform p, float length, int platformType)
		{
		...
		public override void Draw()
		{
			...
			Texture2D side = platformSide[platformType];
			GameMain.MissionSpriteBatch.Draw(Image, Centre, new Rectangle(0, 0, (int)length, 12), Color.White * alpha, GetRadian(Rotation), new Vector2(length / 2, 6), size, SpriteEffects.None, 0.35f);
			Depth = 0.35F;
