...
public partial class Player : Entity
{
	...
	public partial class Heart : Entity
	{
		...
		internal class PurpleFiller : Entity
		{
			...
			{
				CollideRect v = user.controllingBox.CollidingBox;
		...
		/// </summary>
		public FightBox controllingBox;
		...
		public Heart()
		{
			...
			controllingBox = new RectangleBox(this);
			...
			Image.GetData(ColorBitData);
		}
		/// <inheritdoc/>
		public override void Start()
		{
			AddChild(Shields = new());
			GameStates.InstanceCreate(controllingBox);
		...
		public Heart Split()
		{
			...
			v.controllingBox.InstanceMove(controllingBox.CollidingBox);
			isSoulSplit = true;
		...
		public Heart InstantSplit(CollideRect area)
		{
			...
			v.controllingBox.InstanceMove(area);
		...
		private void DoMerge()
		{
			...
			for (int i = 0; i < controllingBox.Vertices.Length; i++)
			{
				controllingBox.Vertices[i].CurrentPosition = controllingBox.Vertices[i].CurrentPosition * v + mergeMission.controllingBox.Vertices[i].MissionPosition * v2;
			}
		...
		#region Precise collision
		/// <summary>
		/// The bounding box of the soul
		/// </summary>
		public Rectangle BoundingBox => GetBoundingBox(BaseRect, GetTransform());
		/// <summary>
		/// The base rectangle of the mask
		/// </summary>
		private Rectangle BaseRect => new(0, 0, Image.Width, Image.Height);
		/// <summary>
		/// Local matrix based on position and rotation
		/// </summary>
		internal Matrix GetTransform(float scale = 1) => Matrix.CreateTranslation(new(-ImageCentre, 0)) * Matrix.CreateScale(scale) * Matrix.CreateRotationZ(Rotation / 180 * float.Pi) * Matrix.CreateTranslation(new(Centre, 0));
		internal static readonly Color[] ColorBitData = new Color[16 * 16];
	...
	public override void Update()
	{
		...
		if (hearts.Count == 0)
			heartInstance = null;
		else if (heartInstance?.Disposed ?? true)
			heartInstance = hearts[0];
	}
	/// <inheritdoc/>
	public override void Draw() { }
