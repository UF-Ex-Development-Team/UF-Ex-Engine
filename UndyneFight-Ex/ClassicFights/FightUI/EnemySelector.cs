namespace UndyneFight_Ex.Fight
{
	internal static class EnemyShowing
	{
		...
		{
			public static vec2 positionDelta;
			private readonly Enemy connected;
			public EnemyShower(string text, float height, Enemy enemy) : base(text, new vec2(50, height))
			{
		...
		internal static FightSelectionCollection GetEnemySelector()
		{
			...
			float curHeight = 256;
			int i = 0;
			foreach (Enemy v in FightStates.enemys)
			{
