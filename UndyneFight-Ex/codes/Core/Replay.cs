...
namespace UndyneFight_Ex
{
    ...
    {
        public Recorder() => records = [];
        public override void Draw() => GlobalResources.Font.NormalFont.CentreDraw("RECORD", new Vector2(320, 60), (playTime % 120 > 59) ? Color.Red : Color.Yellow);
        ...
        private static char FromKeyState(Keys key, KeyboardState state) => state.IsKeyDown(key) ? 'T' : 'F';
        ...
        public void Flush()
        {
            ...
            List<KeyboardState>.Enumerator v = records.GetEnumerator();
            int id = 0;
            while (v.MoveNext())
            {
        ...
        public static void Save() => FlushArea.Record();

        private static class FlushArea
        {
            public static string fileName;
            public static List<byte> fileDatas;
            public static void Record() => IO.IOEvent.WriteCustomFile("Datas\\Records\\" + fileName + ".Tmpf", fileDatas);
        }
    }
    internal class Replayer : Entity
    {
        ...
        public static bool FromInfo(string str, int pos) => str[pos] == 'T';
        ...
        public override void Update()
        {
            ...
            if (appearTime < 0)
                return;
            if (appearTime + det >= states.Length)
            { Dispose(); return; }
