...
namespace UndyneFight_Ex.SongSystem
{
    ...
    public class WaveConstructor : GameObject
    {
        ...
        /// <param name="action">The action to invoke</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ForBeat(float durationBeat, Action action) => AddInstance(new TimeRangedEvent(durationBeat * SingleBeat, action));
        /// <summary>
        ...
        /// <param name="action">The action to invoke</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ForBeat120(float durationBeat, Action action) => AddInstance(new TimeRangedEvent(durationBeat * SingleBeat, action) { UpdateIn120 = true });
        /// <summary>
        ...
        /// <param name="action">The action to invoke</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ForBeat(float delayBeat, float durationBeat, Action action) => AddInstance(new TimeRangedEvent(delayBeat * SingleBeat, durationBeat * SingleBeat, action));
        /// <summary>
        ...
        /// <param name="action">The action to invoke</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ForBeat120(float delayBeat, float durationBeat, Action action) => AddInstance(new TimeRangedEvent(delayBeat * SingleBeat, durationBeat * SingleBeat, action) { UpdateIn120 = true });
        /// <summary>
        ...
        private IEnumerable<GameObject> MakeChartObject(float shootShieldTime, string origin, float speed, ArrowAttribute arrowAttribute, bool normalized)
        {
            ...
            bool GB = origin[0] is '#' or '%';
            bool hasArrowinGB = origin[0] == '#';
        ...
        /// <summary>
        /// Registers a one time function for <see cref="CreateChart(float, float, float, string[])"/> to execute
        /// </summary>
        /// <param name="name">The name of the function</param>
        /// <param name="action">The action to invoke when executed</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// GB：#xx#yz, Where "xx" means the duration beat, "y" beats direction, "z" means color, replace '#' by '%' if you don't want arrows<br/>
            ...
            for (int i = 0; i < Barrage.Length; i++)
            {
                ...
                if (cur.Length > 2)
                {
                    ...
                    if (cur[0] == '!' && cur[1] == '!')
                    {
                        ...
                        {
                            int count = Convert.ToInt32(str[..pos]);
                            currentCount = count;
