...
namespace UndyneFight_Ex.Entities
{
    internal class LoginUI : Selector
    {
        internal class PasswordInputUI : OKCancelSelector
        {
            ...
            public PasswordInputUI(string playerName)
            {
                ...
                OKAction += () =>
                {
                    if (PlayerManager.playerInfos[playerName].Password.ToString() == MathUtil.StringHash(inputer1.Result).ToString())
                    {
            ...
            public override void Draw()
            {
                ...
                NormalFont.Draw("Password:", new Vector2(120, 242), Color.White);
                base.Draw();
            }
        }
        ...
        public LoginUI() : base(true)
        {
            ...
            int y = 0, x = 0;
            foreach (UserService.User info in PlayerManager.playerInfos.Values)
            {
                TextSelection selection = new(info.PlayerName, new Vector2(170 + 300 * x, 100 + 50 * y))
                {
                    TextColor = info.VIP ? Color.Gold : Color.White,
                    SetSelectionAction = () => InstanceCreate(new PasswordInputUI(info.PlayerName))
                };
    ...
    internal class TextInputer : Entity, ISelectAble
    {
        ...
        public override void Draw()
        {
            if (isSelected && appearTime % 66 <= 32)
            {
                FormalDraw(GlobalResources.Sprites.cursor, new Vector2(collidingBox.TopLeft.X + 1 +
                    NormalFont.SFX.MeasureString(Result[..cursorPlace]).X, collidingBox.TopLeft.Y + 4), Color.White, 1.2f, 0.0f, Vector2.Zero);
            }
            if (Result != null)
                NormalFont.Draw(Result, collidingBox.TopLeft + new Vector2(3, 10), Color.LightCoral);
        ...
        public void InputChar(char input)
        {
            ...
            string appro = Result + input;
            float v = NormalFont.SFX.MeasureString(appro).X + 10;
            if (v > CollidingBox.Size.X)
                return;

            Result = Result.Length == 0
                ? input.ToString()
                : Result[..cursorPlace] + input +
                (Result.Length > cursorPlace ?
                Result[cursorPlace..] : "");
        ...
        public override void Update()
        {
            ...
            if (isSelected)
            {
                ...
                if (IsKeyPressed120f(InputIdentity.Backspace))
                {
                    ...
                    if (cursorPlace != 0)
                    {
                        Result = string.Concat(Result.AsSpan(0, cursorPlace - 1),
                                        cursorPlace <= Result.Length ? Result[cursorPlace..] : "");
                        cursorPlace--;
                    }
                }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SelectionEvent() => RegisterUI.ChangeSelection();
    ...
    internal class RegisterUI : Entity
    {
        ...
        public static void ChangeSelection() => needGetDown = true;
        ...
        public override void Draw()
        {
            foreach (ISelectAble v in controls)
            {
                (v as Entity).Draw();
            }
        ...
        public override void Update()
        {
            ...
            if (currentSelect >= controls.Length)
                currentSelect = controls.Length - 1;
            else if (currentSelect < 0)
                currentSelect = 0;
        ...
        {
            public OKButton() : base("Confirm", new Vector2(320, 329)) => Size = 1.0f;
        ...
        {
            public CancelButton() : base("Cancel", new Vector2(320, 369)) => Size = 1.0f;
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public override void SelectionEvent()
            {
                instance.Dispose();
