...
namespace UndyneFight_Ex.Entities
{
    public class BoneSpike : Entity
    {
        ...
        public void RotateTo(float rotation) => rotationDelta = MinRotate(Rotation + rotationDelta, rotation);
        ...
        public override void Draw() => FormalDraw(Image, Centre, drawingColor * alpha, GetRadian(Rotation - 90), ImageCentre);

        public override void Update()
        {
            if (alpha < 1 && appearTime <= 12)
                alpha += 0.1f;
        ...
        public void GetCollide(Player.Heart Heart)
        {
            ...
            if ((colorType == 1 && Heart.IsStable) || (colorType == 2 && Heart.IsMoved))
                return;
            if (res < 0)
            {
                if (!hasHit)
