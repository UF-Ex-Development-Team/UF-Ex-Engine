...
namespace UndyneFight_Ex.Debugging
{
    internal partial class DebugWindow
    {
        ...
        internal static class TypeMatchingLibrary
        {
            ...
            {
                headWords_0_1_0 = ["InsCreate", "InsKill", "Variable", "Exit", "User"];
                keyWords_0_1_0 = ["int", "float", "double", "string", "vector2", "color"];
                operators_0_1_0 = ["+", "-", "*", "/", "+=", "-=", "*=", "/=", "=", "%"];
                splitters_0_1_0 = [' ', ',', ';', '{', '}', '[', ']', '(', ')', '\'', '\"'];
            ...
            {
                if (HeadWords.Contains(data))
                    return WordType.Head;
                if (KeyWords.Contains(data))
                    return WordType.KeyWord;
                string[] patterns = [.. TypeMatches.Keys];
                ...
                if (data.Length == 1)
                    if (Splitters.Contains(data[0]))
                        return WordType.Splitter;
                return Operators.Contains(data) ? WordType.Operator : WordType.Unknown;
            }
        }
        internal static WordType TypeChecking(string data) => TypeMatchingLibrary.Check(data);
    }
}
