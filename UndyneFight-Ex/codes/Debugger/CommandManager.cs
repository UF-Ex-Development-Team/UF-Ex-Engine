namespace UndyneFight_Ex.Debugging
{
    ...
    {
        internal static vec2 CursorPosition { get; private set; }
        internal partial class StringBlock
        {
            ...
            private col DrawingColor => StyleManager.ColorStyleManager.GetColor(_type);
            ...
            public float DrawWithCursor(vec2 start, int position, CursorPositionType cursorPositionType)
            {
                ...
                if (cursorPositionType == CursorPositionType.Point && DrawCursor && CommandManager.HasCommand)
                    instance._spriteBatch.Draw(GlobalResources.Sprites.cursor, cursorPos, col.LightSteelBlue);
                return debugFont.SFX.MeasureString(_data).X;
            }
            ...
            public static implicit operator string(StringBlock _string) => _string._data;
        ...
        internal static class CommandManager
        {
            ...
            private static StringBlock CurrentBlock => blockEnumerator1.Value;
            ...
            public static void Insert(string val)
            {
                ...
                if (res)
                {
                    ...
                    LinkedListNode<StringBlock> tmp = blockEnumerator1.Next;
                    if (blockEnumerator1 == MainCommand)
                        MainCommand = tmp;
            ...
            public static void DeleteInCursor()
            {
                ...
                {
                    if (positionInBlock1 == positionInBlock2)
                        return;
            ...
            public static void MouseClick(vec2 pos)
            {

            }
            public static void MouseRelease(vec2 pos)
            {

            }
            internal static void RenderCommand(vec2 startPosition, LinkedListNode<StringBlock> command)
            {
                LinkedListNode<StringBlock> current = command;
                vec2 pos = new(startPosition.X - InternalCommandSystem.RenderOffset, startPosition.Y);
                vec2 realCursorPos = startPosition;
                bool inCursor = false;
                bool isFixed = false;
