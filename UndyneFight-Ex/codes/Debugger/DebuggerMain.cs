...
namespace UndyneFight_Ex.Debugging
{
    internal partial class DebugWindow : Game
    {
        ...
        {
            if (KeyInputManager.IsKeyDown(Keys.Escape))
                Exit();
            ...
            if (KeyInputManager.IsKeyPressed(Keys.Back))
                CommandManager.DeleteBack();
            if (KeyInputManager.IsKeyPressed(Keys.Left))
                CommandManager.CursorLeft();
            if (KeyInputManager.IsKeyPressed(Keys.Right))
                CommandManager.CursorRight();
