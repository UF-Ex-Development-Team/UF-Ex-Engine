...
public static class FightResources
{
	...
	public static async void Initialize(ContentManager loader)
	{
		Task task = new(()=>
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				if (i < 5)
				{
					...
					if (i < 4)
					{
						...
						Sprites.arrow_base[i] = LoadContent<Texture2D>("bullet\\arrow_base" + (i + 1), loader);
						Sprites.arrow_fore[i] = LoadContent<Texture2D>("bullet\\arrow_fore" + (i + 1), loader);
			...
			for (int i = 0; i < 18; i++)
			{
				Sprites.ParasolMett[i] = LoadContent<Texture2D>($"OtherBarrages\\Mettaton\\spr_parasolmett_{i}", loader);
				if (i < 7)
				{
					Sprites.MettBombCoreBlast[i] = LoadContent<Texture2D>($"OtherBarrages\\Mettaton\\spr_plusbomb_coreblast_{i}", loader);
					Sprites.MettBombBlast[i] = LoadContent<Texture2D>($"OtherBarrages\\Mettaton\\spr_plusbomb_blast_{i}", loader);
					if (i < 2)
						Sprites.MettBomb[i] = LoadContent<Texture2D>($"OtherBarrages\\Mettaton\\spr_plusbomb_{i}", loader);
				}
			}
			...
			Sprites.fireParticle = LoadContent<Texture2D>("others\\fireParticle", loader);
			...
			Sprites.SoulShoot = LoadContent<Texture2D>("SOUL\\soulBullet", loader);
			Sprites.MettBlockA = LoadContent<Texture2D>("OtherBarrages\\Mettaton\\blockA", loader);
			Sprites.MettBlockB = LoadContent<Texture2D>("OtherBarrages\\Mettaton\\blockB", loader);
			Sprites.MettBullet = LoadContent<Texture2D>("OtherBarrages\\Mettaton\\bullet", loader);
			#endregion
			#region Sounds
	...
	public static class Sprites
	{
		...
		public static Texture2D[] arrow_base = new Texture2D[4];
		public static Texture2D[] arrow_fore = new Texture2D[4];
		/// <summary>
		...
		/// </summary>
		public static Texture2D fireParticle;
		/// <summary>
		/// Gun bullet sprite
		/// </summary>
