...
internal class LoginUI : Selector
{
	internal class PasswordInputUI : OKCancelSelector
	{
		...
		public PasswordInputUI(string playerName)
		{
			...
			OKAction += () =>
			{
				if (PlayerManager.playerInfo[playerName].Password.ToString() == MathUtil.StringHash(inputer1.Result).ToString())
				{
	...
	public LoginUI() : base(true)
	{
		...
		int y = 0, x = 0;
		foreach (UserService.User info in PlayerManager.playerInfo.Values)
		{
