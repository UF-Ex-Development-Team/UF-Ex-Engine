...
namespace UndyneFight_Ex.Entities
{
	internal class ChampionShipCard : Entity, ISelectAble
	{
		...
		{
			Centre = startPosition - new Vector2(ChampionShipSelector.XPositionDelta - moveDelta, 10);
			...
			moveDelta = MathF.Round(float.Lerp(moveDelta, isSelected ? 16 : 0, 0.18f), 1);
		}
		...
		public override void Draw()
		{
			...
			DrawingLab.DrawLine(new(collidingBox.Left, collidingBox.Y + BoxMiddle),
				new(collidingBox.Right, collidingBox.Y + BoxMiddle), BoxMiddle * 2, Color.Black, 0.39f);
			DrawingLab.DrawRectangle(collidingBox, isSelected ? Color.Gold : Color.White, 4, 0.4f);
			collidingBox.Height += 10;

			NormalFont.CentreDraw(championShip.Title, Centre + new Vector2(30, -28), Color.White, 1f, 0.53f);
			NormalFont.CentreDraw(championShip.SubTitle, Centre + new Vector2(30, 4), Color.White, 0.8f, 0.53f);
			NormalFont.CentreDraw(championShip.Introduce, Centre + new Vector2(30, 32), Color.White, 0.65f, 0.53f);
		...
		{
#if !DEBUG
            if (championShip.CheckTime() != ChampionShip.ChampionShipStates.End)
            {
                ...
                else if (!PlayerManager.CurrentUser.InChampionShip(championShip.Title))
                {
                    selector.Dispose();
            ...
            if (championShip.CheckTime() == ChampionShip.ChampionShipStates.NotAvailable)
            {
                return;
            }
#endif
	...
	internal class ChampionShipSelector : Selector
	{
		...
		public ChampionShipSelector() : base(true)
		{
			...
			{
#if !DEBUG
                if (s.CheckTime() == ChampionShip.ChampionShipStates.NotStart)
                    continue;
#endif
		...
		public override void Update()
		{
			XPositionDelta = float.Lerp(XPositionDelta, XPositionDeltaMission, 0.2f);
			base.Update();
		}
