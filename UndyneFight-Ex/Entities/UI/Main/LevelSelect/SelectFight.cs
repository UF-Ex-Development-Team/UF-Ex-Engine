...
internal class FightSelector : Selector
{
	private class TextSelectionEx : Entity, ISelectAble
	{
		...
		public class ShinyTextEffect : Entity
		{
			...
			public override void Draw() => FightResources.Font.NormalFont.Draw(texts, Centre - new Vector2(0, size * 14 - 14), showingColor * alpha, size, 0.9f);
	...
	public FightSelector(GameMode mode)
	{
		...
		foreach (Type type in FightSystem.MainGameFights.Values)
		{
			//System.Reflection.Assembly asm = System.Reflection.Assembly.GetExecutingAssembly();
			object tem = Activator.CreateInstance(type);
			TextSelectionEx selection;

			if (tem is Fight.IClassicFight fight)
			{
				string name = fight.FightName;

				selection = new TextSelectionEx(name, new Vector2(100, 160 + 52 * y))
				{
					SetSelectionAction = () => SelectBattle(fight, mode)
				};
			}
			else if (tem is Fight.IExtraOption opt)
			{
				string name = opt.OptionName;
				selection = new TextSelectionEx(name, new Vector2(100, 160 + 52 * y))
				{
					SetSelectionAction = () => ResetScene(Activator.CreateInstance(opt.IntroScene) as Scene)
				};
			}
			else
				throw new ArgumentOutOfRangeException(tem.GetType().Name);
	...
	public override void Draw() => GlobalResources.Font.NormalFont.CentreDraw("Select a fight", new Vector2(320, 45), Color.White * alpha);
