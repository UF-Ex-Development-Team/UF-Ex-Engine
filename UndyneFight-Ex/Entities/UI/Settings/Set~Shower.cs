...
namespace UndyneFight_Ex.Settings
{
	internal class SettingsShower : Selector
	{
		...
		private class MenuSelections
		{
			...
			{
				public CancelButton() : base("Cancel", new Vector2(160, 440)) => Size = 0.9f;
			...
			private class ApplyButton : TextSelection
			{
				...
				public override void SelectionEvent() => shower.Save();
			}

			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static List<TextSelection> GetMenuSelections(SettingsShower shower) =>
				[new CancelButton(), new BackButton(shower), new ApplyButton(shower)];
		}
		...
		public SettingsShower()
		{
			...
			foreach (Type v in SettingsManager.Settings)
			{
				PushSelection(Activator.CreateInstance(v, startPosition) as Setting);
				startPosition.Y += 40;
			}
			foreach (TextSelection v in MenuSelections.GetMenuSelections(this))
				PushSelection(v);
			...
			SelectChanger += () =>
			{
				...
				{
					if (currentSelect == 0)
						currentSelect = SelectionCount - 2;
					else if (currentSelect >= SelectionCount - 3)
						currentSelect = SelectionCount - 4;
					else
						currentSelect--;
				}
				if (GameStates.IsKeyPressed120f(InputIdentity.MainDown))
				{
					if (currentSelect == SelectionCount - 4)
						currentSelect = SelectionCount - 2;
					else
						currentSelect++;
				}
				if (GameStates.IsKeyPressed120f(InputIdentity.MainLeft) && currentSelect >= SelectionCount - 2)
					currentSelect--;
				if (GameStates.IsKeyPressed120f(InputIdentity.MainRight) && currentSelect >= SelectionCount - 3)
					currentSelect++;
