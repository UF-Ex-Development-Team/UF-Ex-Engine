...
public static class SimplifiedEasing
{
	...
	/// <returns>The result of the easing</returns>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static EaseUnit<float> SineWave(float amplitude, float T, float waveCount = 99999, float phase = 0)
	{
		float sine(float t) => float.Lerp(-amplitude, amplitude, MathF.Sin((t / T + phase) * MathF.PI * 2) * 0.5f + 0.5f);
