...
namespace UndyneFight_Ex.Entities
{
	...
	public static class SimplifiedEasing
	{
		...
		{
			Bstate ??= Astate;
			float midx = float.Lerp(start.X, end.X, amount);
			float midy = float.Lerp(start.Y, end.Y, amount);
		...
		{
			Bstate ??= Astate;
			float mid = float.Lerp(start, end, amount);
			EaseUnit<float> A;
			A = Astate switch
			{
				EaseState.Linear => EaseIn(time / 2, start, mid, EaseState.Linear),
		...
		{
			float sine(float t) => float.Lerp(start, end, MathF.Sin((t / T + phase) * MathF.PI * 2) * 0.5f + 0.5f);
			return new((start + end) / 2f, sine(waveCount), waveCount * T, (s) => sine(s.AppearTime));
		}
		/// <summary>
		/// Returns a sine wave easing
		...
		{
			float sine(float t) => float.Lerp(-impact, impact, MathF.Sin((t / T + phase) * MathF.PI * 2) * 0.5f + 0.5f);
	...
	public static class EasingUtil
	{
		...
		public static class ValueEasing
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static Func<ICustomMotion, float> LerpTo(float start, float scale, Func<ICustomMotion, float> origin) => (s) => float.Lerp(start, origin.Invoke(s), scale);
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
