...
public class MiscUtil
{
	...
	public static string GetShorthandName(object thing)
	{
		...
		return (attributes.Length > 0) ? ((ShorthandName)attributes[0]).Name : thing.ToString();
	}
	#endregion
	...
	public class TextBuilder(string text, TextTyper typer = null, char tag_start = '[', char tag_end = ']') : Entity
	{
		...
		/// </summary>
		private readonly (char Open, char Close) CommandTag = (tag_start, tag_end);
		/// <summary>
		/// A double tag for the parser to recognize as a typed out open tag, stored to prevent string concat per parse
		/// </summary>
		private readonly string _doubleCommandTag = $"{tag_start}{tag_start}";
		/// <summary>
		...
		/// </summary>
		private readonly List<GlyphData> _glyphData = [];
		/// <summary>
		/// The list of text sizes, each element contains the width and height
		/// </summary>
		private readonly List<(float Width, float Height)> TextLineSize = [];
		/// <summary>
		...
		private void ParseText()
		{
			...
			{
				int commandTagStartPos = ParsedText.IndexOf(CommandTag.Open);
				int commandTagEndPos = ParsedText.IndexOf(CommandTag.Close) + 1;
				int doubleCommandPos = ParsedText.IndexOf(_doubleCommandTag);
				...
				int nextCommandTagPos = ParsedText[(commandTagStartPos + 1)..].IndexOf(CommandTag.Open) + 1 + commandTagStartPos;
				while (nextCommandTagPos < commandTagEndPos && nextCommandTagPos != commandTagStartPos + 1 && commandTagEndPos != -1)
				{
					...
					nextCommandTagPos = ParsedText[commandTagStartPos..].IndexOf(CommandTag.Open) + 1 + commandTagStartPos;
					commandTagStartPos--;
				}
				string stringBeforeCommand = ParsedText[..commandTagStartPos];
				string stringAfterCommand = ParsedText[commandTagEndPos..];
				//A command is in-lined, format will be [command,argument,argument]
			...
			ParsedText = ParsedText.Replace('\b', CommandTag.Close);
			//Add sprite char if sprite is at the end of text
			if (CommandQueue.Contains(ParsedText.Length) && Commands[CommandQueue.ToList().IndexOf(ParsedText.Length)] == "Sprite")
				ParsedText += '\u0000';
			PostParseAlignment();
		...
		private void ParseCommand(string command_text)
		{
			...
			switch (command_text)
			{
				case "/":
					...
					//Set variables that are effect independent
					currentDrawingData.curEffCount = curEffCount;
				...
				case "Color":
					...
					TextArgument ColorArg = Arguments[CurrentCommandCount];
					//Allocate array
				...
				case "Scale":
					...
					string[] ScaleArg = Arguments[CurrentCommandCount].Arguments;
				...
				case "/Scale":
					...
					break;
				case "Font":
					string FontArg = Arguments[CurrentCommandCount].Arguments[0];
				...
				case "Sprite":
					if (isDrawing)
						break;
					TextArgument SpriteArg = Arguments[CurrentCommandCount];
					...
					drawingPositions.Add(drawingPositions[_currentDrawingCharIndex - 1] + new Vector2(_glyphData[_currentDrawingCharIndex - 1].GetSize().X, 0));
					break;
				case "Wave":
					currentDrawingData.Effect ^= Effects.Wave;
					if (!isDrawing)
					{
						string[] WaveArg = Arguments[CurrentCommandCount].Arguments;
				...
				case "Shake":
					...
					if (!isDrawing)
					{
						string[] ShakeArg = Arguments[CurrentCommandCount].Arguments;
		...
		private void PostParseAlignment()
		{
			...
			for (int i = 0; i < ParsedText.Length; i++)
			{
				...
				if (ParsedText[i] != '\u0000')
				{
					_glyphData.Add(new()
					{
						Blend = currentDrawingData.Col,
						Font = currentDrawingData.CurrentFont,
						Scale = currentDrawingData.Scale,
						_char = ParsedText[i]
					});
					...
					if (LineBroken = ParsedText[i] != '\n')
						maxCharHeight = float.Max(maxCharHeight, charSize.Y * currentDrawingData.Scale.Y);
				}
				else
				{
					LineBroken = false;
					...
					string SpriteToDraw = SpriteArg[0];
					int spriteIndex = SpriteArg.Length > 3 ? (int)MathUtil.FloatFromString(SpriteArg[3]) : 0;
					if (!DefinedTextures.TryGetValue(SpriteToDraw, out Texture2D tex))
					{
						...
						if (preTex is not null)
							preTex = preTex is Texture2D ? preTex as Texture2D : (preTex as Texture2D[])[spriteIndex];
						else //Check FightResources
							if ((preTex = GetAsset(SpriteToDraw, typeof(FightResources.Sprites))) is not null)
								preTex = preTex is Texture2D ? preTex as Texture2D : (preTex as Texture2D[])[spriteIndex];
						tex = preTex as Texture2D;
					}
					_glyphData.Add(new()
					{
						Blend = currentDrawingData.Col,
					...
					_parseDrawingPosition += _glyphData[_currentDrawingCharIndex].GetSize() + new Vector2(4, 0);
					continue;
				}
				_parseDrawingPosition.Y = currentDrawingData.Position.Y;
				drawingPositions.Add(_parseDrawingPosition);
				//Parse line breaking
				vec2 prevCharSize = i == 0 || ParsedText[i - 1] == '\u0000' ? Vector2.Zero : currentDrawingData.CurrentFont.SFX.MeasureString(ParsedText[i - 1].ToString());
				if (ParsedText[i] == '\n')
				{
					...
					TotalTextHeight += currentDrawingData.CurrentFont.SFX.MeasureString(ParsedText[^1..]).Y * currentDrawingData.Scale.Y;
					maxCharHeight = 0;
					...
					TextLineSize.Add((_parseDrawingPosition.X + prevCharSize.X * currentDrawingData.Scale.X * mult, charYDisplace));
			...
			float maxHeight = 0;
			for (int i = int.Max(0, ParsedText.LastIndexOf('\n')); i < ParsedText.Length; i++)
				maxHeight = float.Max(maxHeight, _glyphData[i].GetSize().Y);
			TextLineSize.Add((_parseDrawingPosition.X + charSize.X * currentDrawingData.Scale.X * mult, maxHeight));
		}
		[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
		private Vector2 ParseRenderPosition(int index)
		{
			...
			{
				TextHAlignment.Middle => TextLineSize[_curDrawingLine].Width / 2f,
				TextHAlignment.Right => TextLineSize[_curDrawingLine].Width,
				_ => 0
			};
		...
		public override void Update()
		{
			...
			if (Typer?.DelayTimer == 0)
			{
				...
				if (float.Floor(TextDisplayIndex) != float.Floor(prevIndex))
				{
					if (ParsedText[int.Min((int)TextDisplayIndex, ParsedText.Length - 1)] == '\n')
						TextDisplayIndex++;
					//Play audio
		...
		public override void Draw()
		{
			...
			System.Drawing.RectangleF rect = new(-100, -100, GraphicsDeviceManager.DefaultBackBufferWidth * Fight.Functions.ScreenDrawing.ScreenScale + 100, GraphicsDeviceManager.DefaultBackBufferHeight * Fight.Functions.ScreenDrawing.ScreenScale + 100);
			while (i <= charsToDisplay)
			{
				...
				if (i < charsToDisplay)
				{
					GlyphData curGlyphData = _glyphData[i];
					...
					if (curChar == '\u0000')
					{
						...
						Vector2 spritePosDelta = SpriteArg.Length > 1 ? new(
								...
								SpriteArg[2] == "" ? 0 : MathUtil.FloatFromString(SpriteArg[2])) : Vector2.Zero;
						FormalDraw(curGlyphData.Sprite, ParseRenderPosition(_currentDrawingCharIndex) + spritePosDelta, curGlyphData.Blend[0], curGlyphData.Scale, 0, currentDrawingData.VAlignment switch
						{
							TextVAlignment.Top => Vector2.Zero,
							TextVAlignment.Center => new(0, curGlyphData.Sprite.Height / 2f),
							_ => new(0, curGlyphData.Sprite.Height)
						});
						++i;
						continue;
					}
					...
					if (rect.Contains(new System.Drawing.PointF(parsedPos.X, parsedPos.Y)))
						SpriteBatch.DrawString(curGlyphData.Font, curChar.ToString(), parsedPos, curGlyphData.Blend, 0, scaleN: curGlyphData.Scale, layerDepth: Depth);
				}
				++i;
			}
			isDrawing = false;
		...
		private struct GlyphData()
		{
			...
			/// </summary>
			public Color[] Blend = [Color.White, Color.White, Color.White, Color.White];
			/// <summary>
			/// The font used by the character
			/// </summary>
			public GLFont Font = GlobalResources.Font.NormalFont;
			/// <summary>
			...
			/// </summary>
			public char _char;
			public readonly Vector2 GetSize() => (Sprite is null ? Font.SFX.MeasureString(_char.ToString()) : Sprite.Bounds.Size.ToVector2()) * Scale;
		}
		#endregion
		...
		public (TextHAlignment HAlign, TextVAlignment VAlign) Align() => (defaultTextBuilderData.HAlignment, defaultTextBuilderData.VAlignment);
		/// <summary>
	...
	public class TextTyper : Entity
	{
		...
		public override void Update()
		{
			if (DelayTimer > 0)
			{
				DelayTimer = float.Max(0, --DelayTimer);
			}
		}
		#region User callable functions
		/// <summary>
