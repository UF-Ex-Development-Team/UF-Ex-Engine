...
internal class ModeSelector : Selector
{
	...
	public class ChallengeMode : TextSelection
	{
		...
		public override void SelectionEvent()
		{
			if (selector.selectedMode != 0)
				return;
			//InstanceCreate(new ChallengeSelector());
			ResetScene(new ChallengeScene(CurrentScene, GameStartUp.MainSceneIntro));
			selector.Dispose();
			base.SelectionEvent();
		}
...
internal class SongSelector : Selector
{
	private class TextSelectionEx : Entity, ISelectAble
	{
		...
		public static void MoveUp() => missionDelta += new Vector2(0, 52);
		public static void MoveDown() => missionDelta += new Vector2(0, -52);
		...
		public class ShinyTextEffect : Entity
		{
			...
			public override void Draw() => FightResources.Font.NormalFont.Draw(texts, Centre - new Vector2(0, size * 14 - 22), showingColor * alpha, size, 0.9f);
		...
		public override void Draw()
		{
			...
			FightResources.Font.NormalFont.Draw(texts, startLocation + currentDelta - new Vector2(0, size1 * 14 - 22), showingColor * alpha, size1, 0.9f);
		}
	...
	private readonly List<BackGround> backs = [];
	private readonly GameMode mode;
	private readonly bool customsExist;
	private int songNum;
	private float RectPos = 0;
	private readonly int RectHeight = 25;
	...
	public override void Start()
	{
		...
		int y = 0;
		songNum = 0;
		foreach (Type type in FightSystem.CurrentSongs.Values)
		{
			songNum++;
			...
			if (!Exists(Path.Combine($"{AppContext.BaseDirectory}{filePath}\\song.xnb".Split('\\'))))
			{
				if (!Exists(Path.Combine($"{AppContext.BaseDirectory}{filePath}.xnb".Split('\\'))))
				{
					available = false;
					name = $"({name})";
				}
	...
	public override void Update()
	{
		RectPos = float.Lerp(RectPos, RectPosTarget, 0.16f);
...
internal class DifficultySelector : Selector
{
	...
	{
		public JudgeSelector() => UpdateIn120 = true;
		...
		public JudgementState JudgeState { get; private set; } = JudgementState.Balanced;
	}
	...
	public DifficultySelector(IWaveSet wave)
	{
		...
		SelectChanged += () => PlaySound(changeSelection, 0.9f);
            ...
            if (InChampionShip)
            {
                string _type = PlayerManager.CurrentUser != null
                    ? PlayerManager.CurrentUser.ChampionShipDiv(FightSystem.CurrentChampionShip.Title)
		...
		{
			IChampionShip current = SongSelector.SelectedChampionShip;
			float height = 0;
			HashSet<Difficulty> unlocked = null;
			if (this.wave.Attributes != null)
				unlocked = this.wave.Attributes.UnlockedDifficulties;
			foreach (KeyValuePair<string, Difficulty> v in current.DifficultyPanel)
			{
				if (unlocked != null && !unlocked.Contains(v.Value))
					continue;
	...
	private static Color GetColor(int v) => v switch
	{
		0 => Color.White,
		1 => Color.LawnGreen,
		2 => Color.LightBlue,
	...
	private class DivisionInformation : TextSelection, IGetDifficulty
	{
		...
		public override void SelectionEvent() => base.SelectionEvent();
	}
	...
	public override void Draw()
	{
		if (!string.IsNullOrEmpty(PlayerManager.currentPlayer))
		{
			...
			GLFont f = GlobalResources.Font.NormalFont;
			string Percentage = acc > 0 ? (MathUtil.FloatToString(acc) + "%") : "N/A";
			f.CentreDraw($"Max score: {score}({Percentage})", new Vector2(320, 381), Color.White * alpha);
		...
		{
			FormalDraw(_songIllustration, new(320, 240), Color.White * 0.3f, new Vector2(640 / _songIllustration.Width, 480 / _songIllustration.Height), 0, new(320, 240));
		}
		base.Draw();
	}
	...
	private string Abbreviation(string origin, int timer)
	{
		string finalDisp = string.Empty;
		int MinIndex = MathHelper.Clamp(timer > 90 ? (timer - 90) / 15 : 0, 0, origin.Length - 13);
		if (MinIndex == origin.Length - 13)
			buffertime++;
		if ((timer - 90) / 15 >= origin.Length + 15)
		{
			time = 0;
			buffertime = 0;
		}
		//Dot padding
		if (origin.Length > 16 && MinIndex > 0)
			for (int k = 0; k < MathF.Min(3, MinIndex); k++)
				finalDisp += ".";
		finalDisp += origin.Length > 16 ? origin[MinIndex..(13 + MinIndex)] : origin;
		if (finalDisp != origin)
			for (int k = 0; k < 3 - MathF.Min(3, MinIndex); k++)
				finalDisp += ".";
		return finalDisp;
	}
	private void DrawInformation()
	{
		...
		if (!SongSelector.IsRaceSong)
		{
			GlobalResources.Font.NormalFont.CentreDraw($"{DisplayName} {difficultyNames[currentSelect]}", new Vector2(320, 345), Color.White * alpha);
		}
		else
		{
			if (FightSystem.CurrentChampionShip != null && FightSystem.CurrentChampionShip.CheckTime() == ChampionShipStates.Starting)
			{
				...
				GlobalResources.Font.NormalFont.CentreDraw($"{DisplayName} div. {text}", new Vector2(320, 345), Color.White * alpha);
			}
			else
				GlobalResources.Font.NormalFont.CentreDraw($"{DisplayName} {divNames[currentSelect]}", new Vector2(320, 345), Color.White * alpha);
		}
	}
	...
	private int ChampionShipDifficulty()
	{
		...
		Dictionary<string, Difficulty> panel = SongSelector.SelectedChampionShip.DifficultyPanel;
	...
	private static Color MarkColor(SkillMark mark) => mark switch
	{
	...
	private void DrawACAP(SongData.SongState info)
	{
		if (info.AP)
		{
			float TextY = 417 + Sin(appearTime) * 10;
			float Angle = Sin(appearTime * 0.21f) * 0.25f;
			for (int i = 0; i < 4; i++)
			{
				GlobalResources.Font.NormalFont.CentreDraw("All Perfect",
					...
					new Color(DrawingLab.HsvToRgb(appearTime * 1.3f + i * 60, 255, 255, 255)) * alpha * 0.45f, 1.0f, Angle, 0.1f);
			}
