	...
	internal partial class ResultShower : Entity
	{
		...
		private void RatingDraw()
		{
			...
			if (RatingSelection == 0)
			{
				...
				NormalFont.Draw($"{FloatToString(Rating.X, 2)}", new Vector2(520, 326), Color.Lerp(Color.Transparent, Color.PowderBlue, alpha), 1.2f, 0.4f);
			}
			else
			{
				if (AC)
				{
					...
					NormalFont.Draw($"{FloatToString(Rating.Y, 2)}", new Vector2(520, 197), Color.Lerp(Color.Transparent, Color.MediumPurple, alpha), 1.2f, 0.4f);
				}
				if (AP)
				{
