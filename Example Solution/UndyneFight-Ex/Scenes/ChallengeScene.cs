...
/// <summary>
/// The scene when the player wins a challenge
/// </summary>
internal class ChallengeWinScene : Scene
{
	...
	{
		IsInChallenge = false;
		//Save player challenge data
		if (PlayerManager.CurrentUser is null)
			return;
...
/// <summary>
/// Default challenge selection scene
/// </summary>
public class ChallengeScene(Scene previous_scene, Action exit_event) : Scene
{
	...
	public override void Update()
	{
		...
		for (int i = 0; i < CardPosition.Count; i++)
		{
			...
			CardAlpha[i] = float.Lerp(CardAlpha[i], i == SelectedChallenge ? 1 : 0.5f, 0.1f);
		}
	...
	public override void Draw()
	{
		...
		for (int i = ChallengeList.Count - 1; i >= 0; i--)
		{
			...
			drawFont.CentreDraw(ChallengeName, ThisCardPosition + new Vector2(5, -120), isSelected ? Color.White : Color.Lerp(Color.White, Color.Black, CardAlpha[i]), MathF.Min(0.7f, (CardSize[i].X - 30) / drawFont.SFX.MeasureString(ChallengeName).X) * fontScale, 1);
			drawFont.CentreDraw(curChallenge.Desc, ThisCardPosition - new Vector2(0, 90), TargetColor, MathF.Min(1, (CardSize[i].X - 20) / drawFont.SFX.MeasureString(curChallenge.Desc).X) * fontScale, 1);
			FormalDraw(CardIcons.ElementAt(i), ThisCardPosition - new Vector2(45, 70), TargetColor * TextAlpha, 0, Vector2.Zero);
			//Best score
			if (PlayerManager.CurrentUser?.ChallengeData.AllData.TryGetValue(curChallenge.Title, out SingleChallenge challenge) ?? false)
			{
				float scorePercent = challenge.TripleAccuracy;
				Vector2 pos = ThisCardPosition + new Vector2(5, 70);
				Localization.DrawLocalizedText("Challenge.BestScore", pos, [(scorePercent * 100f).ToString("F2")], color: TargetColor * TextAlpha, depth: Depth + 1e4f, align: Localization.DrawAlign.Middle);
				SkillMark mark = (scorePercent /= 3f) switch
				{
					>= 0.997f => SkillMark.Impeccable,
					>= 0.99f => SkillMark.Eminent,
					>= 0.98f => SkillMark.Excellent,
					>= 0.96f => SkillMark.Respectable,
					>= 0.9f => SkillMark.Acceptable,
					>= 0.75f => SkillMark.Ordinary,
					_ => SkillMark.Failed
				};
				Localization.DrawLocalizedText($"SkillMark.{mark}", pos + new Vector2(0, 50), color: mark switch
				{
				...
				} * TextAlpha * CardAlpha[i], depth: Depth + 1e4f, align: Localization.DrawAlign.Middle);
			}
			else
				Localization.DrawLocalizedText("Challenge.NoData", ThisCardPosition + new Vector2(5, 90), color: Color.Red * TextAlpha * CardAlpha[i], depth: Depth + 1e4f, align: Localization.DrawAlign.Middle);
			//Challenge Details
			if (TextAlpha == 1)
				continue;
			for (int k = 0; k < ChallengeCharts[SelectedChallenge].Count; k++)
			{
				(string Name, Difficulty Difficulty, float Complex) curSelChallenge = ChallengeCharts[SelectedChallenge][k];
				Color ChartNameCol = curSelChallenge.Difficulty switch
				{
				...
				string ChartName = $"* {curSelChallenge.Name}";
				Vector2 ChartNamePos = new(CardPosition[SelectedChallenge].X - CardSize[SelectedChallenge].X / 2f + 10, 180 + k * 60);
				float ChartNameSize = MathF.Min(1, (CardSize[SelectedChallenge].X - 20) / drawFont.SFX.MeasureString(ChartName).X) * fontScale;
				string ComplexName = Localization.GetText("Challenge.ComplexDiff", curSelChallenge.Complex);
				Vector2 ComplexPos = new(CardPosition[SelectedChallenge].X - CardSize[SelectedChallenge].X / 2f + 10, 210 + k * 60);
				ChartNameSize = MathF.Min(ChartNameSize, (CardSize[SelectedChallenge].X - 20) / drawFont.SFX.MeasureString(ComplexName).X);
				for (int j = 0; j < 8; j++)
				{
					drawFont.Draw(ChartName, ChartNamePos + MathUtil.GetVector2(2, j * 45), Color.Black * (1 - TextAlpha), ChartNameSize, 0.99999f);
					drawFont.Draw(ComplexName, ComplexPos + MathUtil.GetVector2(2, j * 45), Color.Black * (1 - TextAlpha), 0.75f * ChartNameSize, 0.99999f);
				}
				drawFont.Draw(ChartName, ChartNamePos, ChartNameCol * (1 - TextAlpha), ChartNameSize, 1);
				drawFont.Draw(ComplexName, ComplexPos, Color.White * (1 - TextAlpha), 0.75f * ChartNameSize, 1);
				string ConfirmText = Localization.GetText("Challenge.Begin");
				Localization.GetFont("FightFont").CentreDraw(ConfirmText, new Vector2(320, 360), Color.LightGray * (1 - TextAlpha), MathF.Min(1, (CardSize[SelectedChallenge].X - 20) / drawFont.SFX.MeasureString(ConfirmText).X) * fontScale, 1);
			}
			//Background images
			float displacement = (CardSize[SelectedChallenge].X - 10) / 4;
			float xBase = CardPosition[SelectedChallenge].X - CardSize[SelectedChallenge].X / 2f + 5;
			Color paintCol = Color.White * 0.5f * (1 - TextAlpha);
			for (int j = 0; j < 3; j++)
			{
				if (Illustrations[SelectedChallenge][j] is null)
					continue;
				SpriteBatch.DrawVertex(Illustrations[SelectedChallenge][j], 0.85f,
					new VertexPositionColorTexture(new Vector3(xBase + displacement * TopIndices[j], 95, 0.85f), paintCol, new Vector2(TopIndices[j] / 4f, 0)),
					new VertexPositionColorTexture(new Vector3(xBase + displacement * TopIndices[j + 1], 95, 0.85f), paintCol, new Vector2(TopIndices[j + 1] / 4f, 0)),
					new VertexPositionColorTexture(new Vector3(xBase + displacement * BottomIndices[j + 1], 385, 0.85f), paintCol, new Vector2(BottomIndices[j + 1] / 4f, 1)),
					new VertexPositionColorTexture(new Vector3(xBase + displacement * BottomIndices[j], 385, 0.85f), paintCol, new Vector2(BottomIndices[j] / 4f, 1)));
			}
		}
		#endregion
		#region Side text
		for (int i = 0; i < 3; i++)
			Localization.DrawLocalizedText($"Challenge.SideTexts[{i}]", new Vector2(10, 340 + i * 30), color: Color.Red);
		#endregion
		#region Overlay UI
		Time++;
