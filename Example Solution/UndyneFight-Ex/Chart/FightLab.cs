...
public static partial class Functions
{
	...
	public static class BoxStates
	{
		...
		/// </summary>
		public static RectangleBox CurrentRectBox => FightBox.instance as RectangleBox;
		/// <summary>
		/// The x coordinate of the left side of the box
		/// </summary>
		public static float Left => CurrentRectBox.Left;
		/// <summary>
		/// The x coordinate of the right side of the box
		/// </summary>
		public static float Right => CurrentRectBox.Right;
		/// <summary>
		/// The y coordinate of the upper side of the box
		/// </summary>
		public static float Up { get => CurrentRectBox.Up; set => CurrentRectBox.Up = value; }
		/// <summary>
		/// The y coordinate of the lower side of the box
		/// </summary>
		public static float Down { get => CurrentRectBox.Down; set => CurrentRectBox.Down = value; }
		/// <summary>
		/// The center of the box
		/// </summary>
		public static Vector2 Centre
		{
			get => CurrentRectBox.Centre; set
			{
				CollideRect area = CurrentRectBox.CollidingBox;
				area.SetCentre(value);
				CurrentRectBox.InstanceMove(area);
			}
		}
		/// <summary>
		/// The width of the box
		/// </summary>
		public static float Width => CurrentRectBox.CollidingBox.Width;
		/// <summary>
		/// The height of the box
		/// </summary>
		public static float Height
		{
			get => CurrentRectBox.CollidingBox.Height;
			set
			{
				CollideRect old = CurrentRectBox.CollidingBox;
				old.Height = value;
				CurrentRectBox.InstanceMove(old);
			}
		}
		/// <summary>
	...
	public static void SetPlayerBoxMission(Player.Heart p)
	{
		...
		SetBoxMission(p.controllingBox);
	}
	/// <summary>
	/// Sets which player and box you are currently controlling
	/// </summary>
	...
	/// <param name="arrowattribute">The arrow attribute of the arrow (Default none), use <see cref="ArrowAttribute"/>, to combine multiple, use 'attr1 | attr2'</param>
	/// <returns>The created <see cref="Arrow"/></returns>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static Arrow MakeArrow(float shootShieldTime, string wayTag, float speed, int color, int rotatingType, ArrowAttribute arrowattribute = ArrowAttribute.None) => MakeArrow(shootShieldTime, GetWayFromTag(wayTag), speed, color, rotatingType, arrowattribute);
	/// <summary>
	/// Creates an arrow with a string as it's tag
	/// </summary>
	/// <param name="shootShieldTime">Time taken to reach the shield</param>
	...
	{
		if (FightBox.instance is RectangleBox rect)
			rect.MoveTo(new CollideRect(x1, y1, x2 - x1, y2 - y1));
		else
	...
	{
		if (FightBox.instance is RectangleBox rect)
			rect.MoveTo(new CollideRect(centre - new Vector2(width, height) / 2, new(width, height)));
		else
			throw new NotImplementedException();
	}
	/// <summary>
	...
	{
		if (FightBox.instance is RectangleBox rect)
			rect.InstanceMove(new CollideRect(centre - new Vector2(width, height) / 2, new(width, height)));
		else
			throw new NotImplementedException();
	}
	/// <summary>
	...
	{
		if (FightBox.instance is RectangleBox rect)
			rect.InstanceMove(new CollideRect(x1, y1, x2 - x1, y2 - y1));
		else
			throw new NotImplementedException();
	}
	/// <summary>
	...
	public static void PlaySound(SoundEffect effect, float soundVolume = 1)
	{
		if (effect is null)
		{
			Debug.WriteLine("Sound is not loaded");
			return;
		}
		if (soundVolume > 1)
		{
