...
public partial class Player
{
	...
	public partial class Heart
	{
		...
		public partial class Shield : Entity, IShieldImage
		{
			...
			internal void Push(GreenSoulGB gb)
			{
				...
				if (Auto || GameStates.IsKeyDown(UpdateKeys[gb.Way]))
					attachedGB = gb;
				if (Auto || PushDelta > user.controllingBox.CollidingBox.Width - 12)
					return;

				PushDelta += 0.1f;
				PushDelta *= 1.2f;
				PushDelta = Math.Min(user.controllingBox.CollidingBox.Width - 12, PushDelta);
			}
			...
			/// </summary>
			public static Color[] ColorTypes { get; set; } = [new(0, 128, 255, 128), new(255, 0, 0, 128), Color.LimeGreen, Color.MediumPurple];
			...
			private protected class ShieldShinyEffect(Entity att, Color cl) : Entity
			{
		...
		public class ShieldManager : Entity
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal Shield GetShield(int shieldID) => shields.TryGetValue(shieldID, out Shield value) ? value : null;
			/// <inheritdoc/>
			public override void Update()
			{
				Heart mission = FatherObject as Heart;
