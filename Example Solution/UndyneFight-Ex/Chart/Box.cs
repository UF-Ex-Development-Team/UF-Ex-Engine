...
public abstract class FightBox : Entity
{
	...
	public FightBox(Player.Heart heart = null)
	{
		UpdateIn120 = true;
		boxes.Add(this);
		instance = this;
...
public class VertexBox : FightBox
{
	...
	/// <exception cref="ArgumentException">If the argument is not a Vector2[]</exception>
	/// <exception cref="ArgumentOutOfRangeException">If the length of the array is not equal to the number of vertices</exception>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public override void InstanceMove(object v)
	{
		...
		if (temp.Length != Vertices.Length)
			throw new ArgumentOutOfRangeException(null, $"{nameof(v)} must be in same length with vertex count");
	...
	{
		if (scale is < 0 or > 1)
			throw new ArgumentOutOfRangeException(null, $"{nameof(scale)} has to be in [0, 1]");
...
public class RectangleBox : FightBox
{
	...
	public RectangleBox(Player.Heart heart, CollideRect? area = null) : base(heart)
	{
		Vertices = [new(), new(), new(), new()];
	...
	/// </summary>
	/// <param name="cl">The <see cref="CollideRect"/> to move to</param>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
