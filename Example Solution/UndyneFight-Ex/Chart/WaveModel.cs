...
public class WaveConstructor : GameObject
{
	...
	private class BracketTreeNode
	{
		...
		private static void CalculateTimes(BracketTreeNode curNode, string mul)
		{
			...
			{
				string[] splits = mul.Split("..");
				curNode.boundL = TryParseInt(curNode, splits[0]);
				mul = splits[1];
				curNode.boundR = 1;
			}
			int mulInt = TryParseInt(curNode, mul);
	...
	/// <returns></returns>
	/// <exception cref="ArgumentException">If amount of ( doesn't equal to )</exception>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	...
	/// <returns></returns>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private void MakeChartObject(float shootShieldTime, string origin, float speed, ArrowAttribute arrowAttribute, bool normalized)
	{
		//Empty entry
		...
		TryGetObjects(origin, shootShieldTime, ref isFunction);
		if (isFunction)
			return;
		...
		result.ForEach(AddInstance);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private void TryGetObjects(string origin, float delay, ref bool isFunction)
	{
	...
	public void MakeArrows(float shootShieldTime, float speed, string allArrowTag, bool normalized = false)
	{
	...
	public GameObject[] MakeArrows(float shootShieldTime, float speed, string allArrowTag)
	{
		string[] arrowTags = SplitBracket(allArrowTag);
		for (int i = 0; i < arrowTags.Length; i++)
			MakeChartObject(shootShieldTime, arrowTags[i], speed, ArrowAttribute.None, false);
		return [];
	}
	[MethodImpl(MethodImplOptions.AggressiveInlining), Obsolete("Use CreateChart() instead")]
	public void MakeArrows(float shootShieldTime, float speed, string allArrowTag, ArrowAttribute arrowattribute, bool normalized = false)
	{
		string[] arrowTags = SplitBracket(allArrowTag);
		for (int i = 0; i < arrowTags.Length; i++)
		{
