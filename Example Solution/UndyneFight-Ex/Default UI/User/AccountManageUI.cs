...
internal partial class AccountManager : Selector
{
	...
	private void GainMessage()
	{
		...
		info = PlayerManager.playerInfo[name];
		isVIP = info.VIP;
	}
		...
		private class VIPManager : OKCancelSelector
		{
			...
			private static ulong GetValue(string s)
			{
				...
				foreach (char item in s)
					res = res * 10 + Convert.ToUInt64(Math.Abs(item - '0'));
				return res;
			}
	...
	private class ChangeColor : TextSelection
	{
		...
		private static void Change()
		{
			switch (NameColor)
			{
				case "White":
					...
					break;
				case "Blue":
					NameColor = (PlayerManager.PlayerSkill >= 75) ?
					"Orange" : "White";
					break;
				case "Orange":
					NameColor = (PlayerManager.PlayerSkill >= 90) ?
					"Colorful" : "White";
					break;
				case "Colorful":
					NameColor = "White";
		...
		public ChangeColor() : base("Current name color: " + NameColor, new Vector2(320, instance.SelectionCount * 50 + 110)) => MaxSize = 1.2f;
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public override void SelectionEvent()
		{
			Change();
			texts = "Current name color: " + NameColor;
			base.SelectionEvent();
		}
	}
		...
		private class ListText : Selector
		{
			private class DataBox : Entity
			{
				...
				public override void Draw()
				{
					...
					GLFont f = FightResources.Font.NormalFont;
					GLFont f2 = FightResources.Font.FightFont;
			...
			public ListText()
			{
				...
				List<DataBox> dataBoxs =
				[
					new(Color.Lime, positions[0], list.CompleteDonor),
					new(Color.Orange, positions[1], list.FCDonor),
					new(Color.Gold, positions[2], list.APDonor)
				];

				for (int i = 0; i < 7; i++)
				{
					if (list.StrictDonors.Count == 0)
						_ = list.StrictDonors.Add(new("NULL", Difficulty.Noob, 0, 0, 0, 0));
					dataBoxs.Add(new(Color.White, positions[i + 3], list.StrictDonors.Max));
					_ = list.StrictDonors.Remove(list.StrictDonors.Max);
				}
	...
	private class ChangePassword : TextSelection
	{
		...
		{
			private readonly TextInputer originPassword, newPassword, passwordAgain;
			public PassWordManager()
			{
				PushSelection(originPassword = new TextInputer(new(270, 160, 260, 38)));
				PushSelection(newPassword = new TextInputer(new(270, 210, 260, 38)));
				PushSelection(passwordAgain = new TextInputer(new(270, 260, 260, 38)));
