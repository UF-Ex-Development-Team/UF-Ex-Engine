	...
	internal partial class ResultShower : Entity
	{
		...
		private static Vector3 SingleCalculateRating(Vector3 Dif, float acc)
		{
			static Tuple<float, float, float> GetDifficulty(IWaveSet waveSet, Difficulty difficulty)
			{
				...
				if (Information != null)
				{
					Information.CompleteDifficulty.TryGetValue(difficulty, out dif1);
					Information.ComplexDifficulty.TryGetValue(difficulty, out dif2);
					Information.APDifficulty.TryGetValue(difficulty, out dif3);
				}
		...
		Vector3 CurrentChartConstRating = Vector3.Zero;
		#endregion
		#region UI
		...
		private void SummaryDraw()
		{
			...
			if (CurrentUser is not null)
			{
				if (CurrentAccuracy > previousAccuracy)
				{
					...
					NormalFont.Draw($"(+{FloatToString((CurrentAccuracy - SongData.SongState.scoreData.PrevAcc) * 100, 1)}%)", new Vector2(516, 127), Color.Lerp(Color.Transparent, Color.Gold, alpha) * (0.8f + Fight.Functions.Sin(appearTime * 4) * 0.2f), 0.5f, 0.5f);
				}
				else
				{
		...
		private float CurrentAccuracy = 0, rerate = 0;
		...
		public override void Update()
		{
			...
			if (gamePlayed.Attributes != null)
			{
				...
				CurrentChartConstRating = SingleCalculateRating(new Vector3(dif[0], dif[1], dif[2]), rerate);
			}
