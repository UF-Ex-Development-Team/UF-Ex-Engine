		...
		private class AnalyzeShow : Entity
		{
			...
			private void Analyze()
			{
				...
				for (int i = 0; i < data.Length; i++)
				{
					...
					if (datum is SoulChangeData soulChangeDat)
						soulChangeData.Add(soulChangeDat);
					else if (datum is SoulListData soulListDat)
					{
						if (!soulListDat.IsInsert)
							soulChangeData.Add(new SoulChangeData(-1, soulListDat.SoulID, soulListDat.Time));
						maximumSoulCount = Math.Max(maximumSoulCount, soulListDat.SoulID + 1);
					}
					else if (datum is ArrowData arrowDat)
						arrowData.Add(arrowDat);
				}
