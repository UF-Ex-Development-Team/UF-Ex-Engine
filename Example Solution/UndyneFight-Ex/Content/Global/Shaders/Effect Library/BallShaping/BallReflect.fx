...
{
	return float2(cos(rad), sin(rad)) * des;
}
...
{
	float2 thisPosition = RenderLocation(mainNoiseUV, input);
	float2 getPos = thisPosition;
	float2 getCenter = mainInfor.x;
	float lenrat = length(thisPosition.xy - getCenter) / mainInfor.x;
	...
	float adj_lenrat = 1.1 - lenrat;
	TextureNow = TextureNow + float4(mainColor.xyz * adj_lenrat * adj_lenrat * adj_lenrat, 1.0);
	
	TextureNow = NormalDraw(TextureNow, float4(mainColor.xyz, 1) * lenrat * lenrat * lenrat); //外壳渲染
	TextureNow.w = min(1.0, floor(mainInfor.x / length(thisPosition.xy - getCenter))); //透明度处理
