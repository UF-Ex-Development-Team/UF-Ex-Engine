...
{
	return (Position.x > SIZESURFACE.x || Position.x < 0.0) || (Position.y > SIZESURFACE.y || Position.y < 0.0) ? 0 : tex2D(samplerTexture, SIZEPIXEL * Position); //SpriteTextureSampler
}

float3 GetPosition(float z)
{
	return float3(sin(z * 0.01) * 20.0, cos(z * 0.01) * 20.0, z);
}
...
float4 MainPS(VertexShaderOutput input) : COLOR
{
	...
	float2 position = v_vPosition - iSize * 0.5;
	...
	for (int i = 0; i < int(amount); i++)
	{
		...
		float2 vecInText = (vecInsur - surCenter).xy + iSize * 0.5;
		float3 color = localToColor(SpriteTextureSampler, vecInText).rgb;
		clr += smoothstep(0., 1., color);
	}
