...
#define PI 3.1415926
//Nope, you cannot set this to a boolean for some reason
uniform int itype;
uniform float idegree;//程度
static const float2 SCREEN = float2(640., 480.), CENTRE = float2(320., 240.);
...
{
	float2 position = bool(itype) ? coordinates_to_polar(input.TextureCoordinates * SCREEN - CENTRE, CENTRE) / SCREEN : polar_to_coordinates(input.TextureCoordinates * SCREEN - CENTRE, CENTRE) / SCREEN;
