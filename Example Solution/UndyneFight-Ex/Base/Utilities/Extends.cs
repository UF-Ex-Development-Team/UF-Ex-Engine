...
public static class Someway
{
	...
	{
		beat *= 8;
		WaveConstructor curWave = (GameStates.CurrentScene as SongFightingScene).waveset as WaveConstructor;
		for (int i = 0; i < eventsname.Length; i++)
			curWave.RegisterFunctionOnce(eventsname[i], events[i]);
		List<string> rhythmList = [.. rhythm];
		bool prevWasFunc = false;
		//Convert to CreateChart standard
		for (int i = 0; i < rhythmList.Count; i++)
		{
			if (eventsname.Contains(rhythmList[i]))
			{
				rhythmList[i] = $"({rhythmList[i]})";
				if (prevWasFunc)
				{
					rhythmList[i - 2] += rhythmList[i];
				...
				prevWasFunc = false;
			if (rhythmList[i].StartsWith('G'))
			{
				rhythmList[i] = $"%{MathUtil.FloatFromString(rhythmList[i][3..]) / 8f}%${MathUtil.FloatFromString(rhythmList[i][1].ToString())}{MathUtil.FloatFromString(rhythmList[i][2].ToString())}";
				rhythmList.Insert(i + 1, "<<0.125");
				i++;
			}
			else if (rhythmList[i].StartsWith('W'))
			{
				rhythmList[i] = $"#{MathUtil.FloatFromString(rhythmList[i][3..]) / 8f}#R{MathUtil.FloatFromString(rhythmList[i][2].ToString())}";
				rhythmList.Insert(i + 1, "<<0.125");
				i++;
			}
		...
		foreach (var v in eventsname)
			textWriter.WriteLine($"RegisterFunctionOnce(\"{v}\", () => {{\n\t\n}});");
		string allChart = string.Empty;
		for (int i = 0; i < rhythmList.Count; i++)
		{
			//Convert empty
			if (rhythmList[i] == "/")
				rhythmList[i] = string.Empty;
			//Merge X <<0.125 Y
			if (i + 2 < rhythmList.Count && rhythmList[i + 1] == "<<0.125")
			{
				if (rhythmList[i + 2] == "/")
					rhythmList[i + 2] = string.Empty;
				if (rhythmList[i + 2] != string.Empty)
					rhythmList[i] += rhythmList[i + 2].StartsWith('(') ? rhythmList[i + 2] : $"({rhythmList[i + 2]})";
				rhythmList.RemoveRange(i + 1, 2);
			}
			//Convert single function with bracket to no bracket
			if (MathUtil.CountSubstring(rhythmList[i], "(") == 1)
				rhythmList[i] = rhythmList[i][1..^1];
			allChart += $"\"{rhythmList[i]}\", ";
			if (i % 8 == 0 && i > 0)
				allChart += "\n\t";
		}
		textWriter.WriteLine($"CreateChart(BeatTime({starttime / beat}), BeatTime({beat / beat}), {(arrowspeed == (int)arrowspeed ? arrowspeed : arrowspeed + "f")}, [\n\t{allChart}\n]);");
		textWriter.Flush();
		textWriter.Dispose();
		stream.Dispose();
#endif
