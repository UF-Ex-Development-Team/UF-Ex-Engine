...
public static class SimplifiedEasing
{
	...
	/// <returns>The result of the easing</returns>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static EaseUnit<Vector2> Copy(EaseUnit<Vector2> ease, int times, bool isAdjust = true)
	{
		EaseUnit<Vector2>[] easeUnits = new EaseUnit<Vector2>[times];
		for (int i = 0; i < times; i++)
			easeUnits[i] = new(ease.Start, ease.End, ease.Time, ease.Easing);
		return LinkEase(isAdjust, easeUnits);
	}
	/// <summary>
	/// Returns a copy of the specified easing for the given amount of times
	/// </summary>
	...
	/// <returns>The result of the easing</returns>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static EaseUnit<float> Copy(EaseUnit<float> ease, int times, bool isAdjust = true)
	{
...
public static class EasingUtil
{
	...
	public static class ValueEasing
	{
		...
		/// <returns></returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Func<ICustomMotion, float> SinWave(float intensity, float cycleTime, float startPhase) => (s) => Sin((s.AppearTime / cycleTime + startPhase) * 180) * intensity;
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
...
public static class EaseLibrary
{
	...
	public static float GetValue(float timeProgress, EaseState state, EaseMode mode) => GetValue(timeProgress, state.ToString(), mode);
	/// <summary>
	/// Gets the lerp value of the easing at a given time
	/// </summary>
	/// <param name="timeProgress">The progress of the easing, value should be [0, 1]</param>
